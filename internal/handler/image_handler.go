...
type ImageHandler struct {
	svc *service2.ImageService
}

// NewImageHandler 创建 ImageHandler
func NewImageHandler(svc *service2.ImageService) *ImageHandler {
	return &ImageHandler{svc: svc}
}

// List GET /api/v1/images
func (h *ImageHandler) List(c *gin.Context) {
	var req service2.ImageListRequest
...
func (h *ImageHandler) Inspect(c *gin.Context) {
	...
	if err != nil {
		respondJSON(c, service2.Error(service2.ErrCodeImageNotFound, err.Error()))
		return
	}
...
func (h *ImageHandler) Pull(c *gin.Context) {
	var req service2.ImagePullRequest
...
func (h *ImageHandler) Remove(c *gin.Context) {
	var req service2.ImageRemoveRequest
...
func (h *ImageHandler) Tag(c *gin.Context) {
	var req service2.ImageTagRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		respondJSON(c, service2.Error(service2.ErrCodeInvalidParam, err.Error()))
		return
	}
...
func (h *ImageHandler) Push(c *gin.Context) {
	var req service2.ImagePushRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		respondJSON(c, service2.Error(service2.ErrCodeInvalidParam, err.Error()))
		return
	}
...
func (h *ImageHandler) Import(c *gin.Context) {
	var req service2.ImageImportRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		respondJSON(c, service2.Error(service2.ErrCodeInvalidParam, err.Error()))
		return
	}
	...
	respondJSON(c, service2.Success(gin.H{"message": "请使用 /images/load 接口上传tar文件"}))
}
